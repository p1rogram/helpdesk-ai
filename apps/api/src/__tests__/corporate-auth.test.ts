...
describe('rolesFor', () => {
  it('grants operator by group DN or CN', () => {
    ...
    expect(rolesFor({ id: 'a', displayName: 'a', groups: ['CN=helpdesk-operators,OU=Groups,DC=tpu,DC=ru'] }, ops)).toEqual(['operator']);
...
describe('OidcProvider (fake IdP)', () => {
  ...
  const fakeFetch: typeof fetch = async (input, init) => {
    ...
    if (url.endsWith('/.well-known/openid-configuration')) {
      return new Response(JSON.stringify({ authorization_endpoint: 'https://idp/auth', token_endpoint: 'https://idp/token', userinfo_endpoint: 'https://idp/me' }));
    }
    ...
    if (url === 'https://idp/me') {
      return new Response(JSON.stringify({ preferred_username: 'ivanov', name: 'Иванов Иван', email: 'ivanov@tpu.ru', groups: ['students'] }));
    }
    return new Response('nf', { status: 404 });
  };
  const oidc = new OidcProvider({ issuer: 'https://idp', clientId: 'app', redirectUri: 'https://app/cb', fetchImpl: fakeFetch });
  ...
  it('runs the authorization-code + PKCE flow and maps claims', async () => {
    ...
    expect(identity).toEqual({ id: 'ivanov', displayName: 'Иванов Иван', email: 'ivanov@tpu.ru', groups: ['students'] });
