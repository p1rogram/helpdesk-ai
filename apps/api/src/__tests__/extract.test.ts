...
const clarify = (catalog.categories.find((c) => c.id === 'campus')?.clarify ?? []) as ClarifyingField[];

describe('deterministic field extraction', () => {
  it('reads the dorm number out of a question', () => {
    expect(extractFields(clarify, 'Какой адрес у общежития 12 и кто заведующий?').fields.building).toBe(
      'общежитие №12',
    );
  });

  it('reads the number when it comes before the word', () => {
    expect(extractFields(clarify, 'я живу в 14 общежитии, не работает душ').fields.building).toBe('общежитие №14');
  });
  ...
  it('stays silent when nothing is named', () => {
    expect(extractFields(clarify, 'не работает интернет')).toEqual({ fields: {}, reject: undefined });
  });
