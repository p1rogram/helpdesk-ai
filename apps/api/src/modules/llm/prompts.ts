...
export function analyzeSystemPrompt(catalog: LoadedCatalog): string {
  ...
  return [
    ...
    `- smalltalkReply — только при offTopic: 1–2 живых, тёплых предложения на языке пользователя, как ответил бы дружелюбный сотрудник поддержки (не робот, без канцелярита), и мягкое приглашение описать проблему. Пример на «ты тут?»: «Да, на месте! Расскажите, что случилось — постараюсь помочь.» Если вне темы задан вопрос или просьба (посчитать буквы, написать стих, рассказать анекдот, «кто ты», что-то про мир) — НЕ отвечай на него по существу и не выполняй: только дружелюбно вернись к теме поддержки. Ты не универсальный ассистент.`,
...
export function solveUserPrompt(input: {
  ...
  return [
    ...
    input.passages?.length
      ? `\nФрагменты документации — только для уточнения фактов (адресов, телефонов, ссылок, сроков) по теме статьи; шаги по-прежнему из статьи. Фрагменты о других программах, выплатах или услугах, о которых пользователь не спрашивал, игнорируй:\n${renderPassages(input.passages)}`
      : '',
  ]
    .filter(Boolean)
    .join('\n');
...
export function answerSystemPrompt(catalog: LoadedCatalog): string {
  return [
    ...
    `- Отвечай на заданный вопрос. Если фрагменты описывают несколько программ или видов выплат, говори о той, про которую спросили; специальные программы (например, отдельные стипендии конкретного направления) упоминай только когда пользователь спрашивает о них.`,
